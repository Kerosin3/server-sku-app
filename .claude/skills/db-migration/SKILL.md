...
2. `alembic/versions/0001_initial_schema.py` — источник истины по
   ...
   Исключение: пока нет ни одного реального (не одноразового тестового)
   развёртывания на этой схеме, можно продолжать редактировать 0001
   напрямую вместо накопления ревизий поверх ещё не устоявшегося
   дизайна — так и было сделано при переходе на иерархию Платформа →
   Исполнение → Изделие. Как только появится первый реальный деплой —
   это исключение больше не действует.
...
- Если в проекте появилась новая таблица — обновить список таблиц в
  ...
  таблиц (сейчас 12) оставался актуальным.
