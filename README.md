...
Это скелет проекта: схема БД, модели, аутентификация (включая
управление пользователями — `/users`, только для роли `admin`) готовы.
Остальной CRUD-функционал размечен как `TODO(agent)` в роутерах — см.
roadmap в AGENTS.md.
