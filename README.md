...
(`/users`, только `admin`), конструктор моделей платформ
(`/platform-models`, `admin`/`engineer`), CRUD платформ с чек-листом
комплектности и установкой/снятием компонентов по серийному номеру
(`/platforms`). Пока нет CRUD для part_types/part_units — без него
компонент негде завести перед установкой на платформу, это следующий
приоритет. Подробный статус — roadmap в AGENTS.md.
