...
class MacAddress(Base):
    """
    A MAC address belonging either to a platform (e.g. a chassis-level
    BMC/management port) or to a specific part_unit (typically a
    motherboard with several network interfaces: LAN1/LAN2/BMC, or a
    standalone NIC card in a riser).

    Exactly one of platform_id / part_unit_id must be set — this is an
    "exclusive arc", enforced by a DB-level CHECK constraint (num_nonnulls),
    not a soft convention in application code. A polymorphic association
    without real foreign keys is deliberately avoided here — it breaks
    integrity (the ORM/application could forget to check it).
    ...
    platform: Mapped["Platform | None"] = relationship(back_populates="mac_addresses")
